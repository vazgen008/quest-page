<template>
  <div class="quests">
    <p class="quests__title">Pre Reserved Movies`</p>
    <p class="quests__title" v-if="!cards.length">There is no Pre-Reserved Movies</p>


    <div class="quests-flex">
        <div class="quests__movies" v-for="newMovie in cards" :key="newMovie.id">
...
const NewMovies = NewQuests()
const PopCards = popularQuests()

const reserveNewMovie = (newMovie)=>{
   NewMovies.reserveComingMovie(newMovie)
}

onMounted(() => {
    NewMovies.setAllCards(JSON.parse(localStorage.getItem("comingMovies")))
})

const cards = computed(()=>{
    const ResCards = NewMovies.getAllNewMovies
    console.log(ResCards)
    return ResCards.filter((movie) => movie.reserve === true).filter(movie=>movie.title.includes(PopCards.searchText))
})
